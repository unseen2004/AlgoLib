...
template <typename T>
b_tree<T>::node::node(int degree, bool is_leaf)
...
template <typename T>
void b_tree<T>::insert_non_full(node* current_node, const T& key) {
...
template <typename T>
void b_tree<T>::split_child(node* parent, int index, node* child) {
...
template <typename T>
void b_tree<T>::remove_key(node* current_node, const T& key) {
    int idx = 0;
...
template <typename T>
T b_tree<T>::get_predecessor(node* current_node) {
...
template <typename T>
T b_tree<T>::get_successor(node* current_node) {
    while (!current_node->m_is_leaf)
...
template <typename T>
void b_tree<T>::fill_node(node* current_node, int index) {
...
template <typename T>
void b_tree<T>::borrow_from_prev(node* current_node, int index) {
...
template <typename T>
void b_tree<T>::borrow_from_next(node* current_node, int index) {
    node* child = current_node->m_children[index];
    node* sibling = current_node->m_children[index + 1];
...
template <typename T>
void b_tree<T>::merge_nodes(node* current_node, int index) {
    node* child = current_node->m_children[index];
    node* sibling = current_node->m_children[index + 1];
    child->m_keys.push_back(current_node->m_keys[index]);
...
template <typename T>
void b_tree<T>::print_node(node* current_node, int level) const {
...
template <typename T>
bool b_tree<T>::search_node(node* current_node, const T& key) const {
...
template class b_tree<double>;
template class b_tree<std::string>;
