...
private:
    void insert_non_full(BNode *current_node, const T &key);

    void split_child(BNode *parent, int index, BNode *child);

    void remove_key(BNode *current_node, const T &key);

    T get_predecessor(BNode *current_node);

    T get_successor(BNode *current_node);

    void fill_node(BNode *current_node, int index);

    void borrow_from_prev(BNode *current_node, int index);

    void borrow_from_next(BNode *current_node, int index);

    void merge_nodes(BNode *current_node, int index);

    void print_node(BNode *current_node, int level) const;

    bool search_node(BNode *current_node, const T &key) const;
};

template<typename T>
BTree<T>::BNode::BNode(int degree, bool is_leaf)
    : m_degree(degree), m_is_leaf(is_leaf) {
}

template<typename T>
BTree<T>::BTree(int degree) : m_root(nullptr), m_degree(degree) {
}
...
void BTree<T>::insert(const T &key) {
    if (!m_root) {
        ...
        if (m_root->m_keys.size() == 2 * m_degree - 1) {
            BNode *new_root = new BNode(m_degree, false);
...
template<typename T>
void BTree<T>::insert_non_full(BNode *current_node, const T &key) {
...
template<typename T>
void BTree<T>::split_child(BNode *parent, int index, BNode *child) {
    BNode *new_child = new BNode(child->m_degree, child->m_is_leaf);
...
template<typename T>
void BTree<T>::remove_key(BNode *current_node, const T &key) {
    int idx = 0;
...
template<typename T>
T BTree<T>::get_predecessor(BNode *current_node) {
...
template<typename T>
T BTree<T>::get_successor(BNode *current_node) {
    while (!current_node->m_is_leaf)
...
template<typename T>
void BTree<T>::fill_node(BNode *current_node, int index) {
...
template<typename T>
void BTree<T>::borrow_from_prev(BNode *current_node, int index) {
    BNode *child = current_node->m_children[index];
    BNode *sibling = current_node->m_children[index - 1];
...
template<typename T>
void BTree<T>::borrow_from_next(BNode *current_node, int index) {
    BNode *child = current_node->m_children[index];
    BNode *sibling = current_node->m_children[index + 1];
...
template<typename T>
void BTree<T>::merge_nodes(BNode *current_node, int index) {
    BNode *child = current_node->m_children[index];
    BNode *sibling = current_node->m_children[index + 1];
    child->m_keys.push_back(current_node->m_keys[index]);
...
template<typename T>
void BTree<T>::print_node(BNode *current_node, int level) const {
...
template<typename T>
bool BTree<T>::search_node(BNode *current_node, const T &key) const {
